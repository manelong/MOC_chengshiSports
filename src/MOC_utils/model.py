...
def load_coco_pretrained_model(opt, model):
    if opt.arch == 'dla_34':
        ...
        print('load coco pretrained yolo')
        model_path = 'experiment/modelzoo/yolov8_m_MOC.pth'
    else:
        raise NotImplementedError
    checkpoint = torch.load(model_path, map_location=lambda storage, loc: storage)
    if opt.arch != 'yolo':
        ...
        print('load coco pretrained yolo backbone successfully')

    return model

def load_multisports_pretrained_model(opt, model):
    # if opt.arch == 'dla_34':
    #     print('load coco pretrained dla_34')
    #     model_path = '../experiment/modelzoo/coco_dla.pth'
    # elif opt.arch == 'resnet_18':
    ...
    #     raise NotImplementedError
    if opt.arch == 'dla_34':
        print('load multisports pretrained dla_288_512_nn')
        model_path = '../experiment/modelzoo/multisports_dla_288_512_nn.pth'
    checkpoint = torch.load(model_path, map_location=lambda storage, loc: storage)
    print('loaded {}, epoch {}'.format(model_path, checkpoint['epoch']))
    state_dict_ = checkpoint['state_dict']
    state_dict = {}
    ...
    print('load multisport_288_512_agru pretrained successfully')
    if opt.print_log:
        check_state_dict(model.state_dict(), new_state_dict)
        print('check done!')
