...
from ACT_utils.ACT_utils import tubelet_in_out_tubes, tubelet_has_gt, tubelet_near_key_frame
...
class BaseDataset(data.Dataset):
    ...
    def __init__(self, opt, mode, ROOT_DATASET_PATH, pkl_filename):
        ...
        # 改进为关键帧附近的判断
        # 判断是不是项目数据集
        if opt.dataset == 'newbasketball':
            for v in video_list:
                vtubes = sum(self._gttubes[v].values(), [])
                keyframe = self._keyframe[v]
                self._indices += [(v, i) for i in range(1, self._nframes[v] + 2 - self.K)
                                  if tubelet_in_out_tubes(vtubes, i, self.K) and tubelet_has_gt(vtubes, i, self.K) and tubelet_near_key_frame(i, self.K, keyframe)]
        else:
            for v in video_list:
                vtubes = sum(self._gttubes[v].values(), [])
                self._indices += [(v, i) for i in range(1, self._nframes[v] + 2 - self.K)
