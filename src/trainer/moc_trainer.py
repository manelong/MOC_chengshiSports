...
class MOCTrainLoss(torch.nn.Module):
    ...
    def forward(self, output, batch):
        ...
        pos_list = []
        count = 0
        for feature in batch['hm']:
            if torch.max(feature) != 0:
                pos_list.append(count)
            count += 1
        if not pos_list:
            mov_loss = torch.tensor([0.0]).to(opt.device)
            wh_loss = torch.tensor([0.0]).to(opt.device)
        else:
            pos_list = torch.tensor(pos_list).to(opt.device)
            for key in batch:
                if key == 'input':
                    continue
                batch[key] = torch.index_select(batch[key], 0, pos_list)
            for key in output:
                output[key] = torch.index_select(output[key], 0, pos_list)
            # # output['hm'] = torch.index_select(output['hm'], 0, pos_list)
            # # batch['hm'] = torch.index_select(batch['hm'], 0, pos_list)
            # output['mov'] = torch.index_select(output['mov'], 0, pos_list)
            # batch['mov'] = torch.index_select(batch['mov'], 0, pos_list)
            # output['wh'] = torch.index_select(output['wh'], 0, pos_list)
            # batch['wh'] = torch.index_select(batch['wh'], 0, pos_list)
